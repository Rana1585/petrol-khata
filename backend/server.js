...
async function fetchCurrentFuelPrice() {
    ...
    const petrol = data.products.find(
        (item) =>
            item.product ===
            "Motor Spirit (Petrol)"
    );
...
/*
   Everything below this point belongs
...
app.delete(
    ...
    async (req, res) => {
        try {
            ...
            const rows = await db`
                DELETE FROM vehicle_records vr
                USING vehicles v
                WHERE vr.id = ${id}
                  AND v.id =
                      vr."vehicleId"
...
app.delete(
    ...
    async (req, res) => {
        try {
            ...
            const rows = await db`
                DELETE FROM vehicle_maintenance vm
                USING vehicles v
                WHERE vm.id = ${id}
                  AND v.id =
                      vm."vehicleId"
